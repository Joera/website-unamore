import { ALCHEMY_KEY, IPFS_URL } from "./renderer/src/constants";
import { protocolABI } from "../src/libs/abi";
...
export const getProtocolInfo = async () => {
    try {
        ...
        const configResponse = await fetch(`${IPFS_URL}/api/v0/cat?arg=${configCid}`, {
            method: "POST"
        });
...
export const addressFromBytes = (publicKeyBytes: Uint8Array | string): string => {
