...
export const renderHTML = async (config: any, templateConfig: any, templateData: any) => {
    try {
        // Validate inputs
        if (!config?.template_cid || !templateConfig?.file) {
            console.error('Missing required config:', { template_cid: config?.template_cid, file: templateConfig?.file });
            return '';
        }

        // Fetch template array
        const response = await fetch(`${IPFS_URL}/api/v0/dag/get?arg=${config.template_cid}`, {
            method: 'POST'
        });
        ...
        const templateFile = templateArray.find((t: any) => t.path.includes(templateConfig.file));
        ...
        const partialFiles = templateArray.filter( (t: any) => t.path.includes("partials/"));
